...
export class TesteDeNivelamentoComponent {
  ...
  continuaFinalizaTeste() {
    ...
    if (this.perguntasTDN[this.questaoAtual]) {
      ...
      for (let value of this.respostasSalvas) {
        if (
          this.perguntasTDN[value.questao - 1].respostas[
            value.respostaEscolhida - 1
          ].eACorreta
        ) {
          pontuacao++;
        }
  ...
  testeShowResult() {
    if (this.nivelUser == 'iniciante') {
      ...
      this.testeText =
        'Um perfil iniciante na área de finanças é alguém que está nos estágios iniciais de sua jornada de compreensão do mundo financeiro. <br><br> Essa pessoa já possui um entendimento básico de conceitos como orçamento e investimento, mas está no processo de aprofundar esse conhecimento.';
    } else if (this.nivelUser == 'intermediario') {
      this.testeTitle = `Seu nivel é: Intermediário`;
      this.testeText =
        'Um perfil intermediário é de alguém que ja tém um conhecimento bom sobre finanças mas ainda não é um mestre sobre o assunto.';
    } else {
      this.testeTitle = `Seu nivel é: Avançado`;
      this.testeText =
        'PARABÉNS! Seu perfil é avançado <br><br> Você definitivamente sabe sobre finanças';
    }
